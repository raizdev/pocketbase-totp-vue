                        ...
                        <form @submit.prevent="onSubmit">
                            <p>
                                ...
                                <div class="mt-8 flex items-center justify-center">
                                    <QrCodeVue :value="`otpauth://totp/${totp.issuer}:${user.email}?secret=${totp.secret}&issuer=${totp.issuer}`" size="250" v-if="totp.secret"></QrCodeVue>
                                </div>
                            <div>
...
export default {
    ...
    methods: {
        ...mapActions(useAuthStore, ['activateTotpAuth', 'getTotpSecretkey']),

        async onSubmit () {
            this.returnMessage = await this.activateTotpAuth({
    ...
    async created() {
        this.totp = await this.getTotpSecretkey()
    }
}
</script>